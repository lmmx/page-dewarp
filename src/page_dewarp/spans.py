...
from .options import span_opts, edge_opts
...
def generate_candidate_edge(cinfo_a, cinfo_b):
    ...
    if not (
        dist > edge_opts["EDGE_MAX_LENGTH"]
        or x_overlap > edge_opts["EDGE_MAX_OVERLAP"]
        or delta_angle > edge_opts["EDGE_MAX_ANGLE"]
    ):
        score = dist + delta_angle * edge_opts["EDGE_ANGLE_COST"]
...
def assemble_spans(name, small, pagemask, cinfo_list, min_width=None):
    if min_width is None:
        min_width = span_opts["SPAN_MIN_WIDTH"]
...
def sample_spans(shape, spans, span_px_per_step=None):
    ...
    for span in spans:
        ...
        for cinfo in span:
            ...
            step = span_px_per_step if span_px_per_step else span_opts["SPAN_PX_PER_STEP"]
...
def keypoints_from_samples(name, small, pagemask, page_outline, span_points):
    ...
    pagecoords = pix2norm(
        pagemask.shape, pagecoords.reshape((-1, 1, 2))
    ).reshape((-1, 2))
...
def visualize_span_points(name, small, span_points, corners):
    ...
    for i, points in enumerate(span_points):
        ...
        for point in points:
            circle(
                display, fltp(point), 3, cCOLOURS[i % len(cCOLOURS)], -1, LINE_AA
            )
