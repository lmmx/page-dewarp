...
- Calculate centroid and orientation for each contour
- Filter contours by size and shape
- Construct minimal masks
- Visualize resulting contours for debugging
"""
...
def blob_mean_and_tangent(contour: np.ndarray) -> tuple[np.ndarray, np.ndarray] | None:
    ...
    Args:
        contour: A single contour (numpy array).

    Returns:
        A tuple (center, tangent), or None if the contour area is zero.
    ...
    if not area:
        logger.debug("Discarding contour with zero moments")
        return None
...
def interval_measure_overlap(
    ...
    Args:
        int_a: The first interval (start, end).
        int_b: The second interval (start, end).

    Returns:
        The overlap length (may be negative if no overlap).
...
class ContourInfo:
    ...
    def __init__(
        ...
        Args:
            contour: The raw points making up the contour.
    ...
    def local_overlap(self, other: ContourInfo) -> float:
        """Compute the overlap of this contour's local axis range with another's."""
...
def make_tight_mask(
    ...
    Args:
        ...
        xmin, ymin: Top-left corner of the bounding rect.
...
def get_contours(name: str, small: np.ndarray, mask: np.ndarray) -> list[ContourInfo]:
    """Detect and filter contours in a binary mask.
    ...
    Returns:
        A list of ContourInfo objects for contours that pass size/aspect checks.
    ...
    for contour in contours:
        ...
        moments = blob_mean_and_tangent(contour)
        if moments is None:
            continue
    ...
    logger.debug(
        ...
        extra={
            ...
            "after_filtering": len(contours_out),
        },
    )
...
def visualize_contours(
    ...
    """Overlay colored contours on a copy of the image for debugging."""
